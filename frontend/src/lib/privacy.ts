...
function isLoopbackUrl(value: string): boolean {
  ...
  try {
    ...
    const rawHostname = authorityMatch[1].replace(/^\[|\]$/g, "").toLowerCase();
    if (rawHostname === "localhost" || isIpv4Loopback(rawHostname)) return true;
    const parsedHostname = endpoint.hostname.replace(/^\[|\]$/g, "").toLowerCase();
    return rawHostname.includes(":") && isIpv6Loopback(parsedHostname);
  } catch {
    return false;
  }
}
