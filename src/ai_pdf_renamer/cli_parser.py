...
from .cli_parser_sections import (
...
def _add_conflict_resolution_args(p: argparse._ActionsContainer) -> None:
    ...
    p.add_argument(
        ...
        action="store_true",
        help="Use embedding similarity for conflict resolution. Requires a manual sentence-transformers install.",
    )
    p.add_argument(
        "--category-display",
...
def _add_llm_backend_args(p: argparse._ActionsContainer) -> None:
    _add_llm_backend_selection_args(p)
    _add_llm_connection_args(p)


def _add_llm_backend_selection_args(p: argparse._ActionsContainer) -> None:
    p.add_argument(
        "--llm-backend",
        dest="llm_backend",
        default=None,
...
def _add_llm_connection_args(p: argparse._ActionsContainer) -> None:
    p.add_argument(
        ...
        help=(
            ...
            "(default: env AI_PDF_RENAMER_LLM_URL or preset-driven http://127.0.0.1:11434/v1/completions)"
        ),
    )
    p.add_argument(
        "--llm-model",
