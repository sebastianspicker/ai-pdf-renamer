...
from .filename_llm_metadata import (
...
from .heuristics import CategoryCombineParams, HeuristicScorer, combine_categories, normalize_llm_category
from .llm import get_document_category
from .llm_backend import LLMClient
from .rules import ProcessingRules
from .text_utils import Stopwords, clean_token, normalize_keywords, split_to_tokens, subtract_tokens
...
def _should_skip_llm_category(
    ...
    return bool(
        ...
        and request.heuristic_score >= config.skip_llm_category_if_heuristic_score_ge
        and request.heuristic_gap >= config.skip_llm_category_if_heuristic_gap_ge
    )
...
def _resolve_llm_category_candidate(
    ...
    *,
    ...
    cat_llm = get_document_category(
        ...
        language=config.language,
        suggested_categories=_suggested_categories_for_llm(request.heuristic_text, config, heuristic_scorer)
        if not allowed
        else None,
...
def _category_overlap_context(request: _CategoryResolutionInput, config: RenamerConfig) -> str | None:
    if not config.use_keyword_overlap_for_category:
        return None
...
def _skip_llm_by_rule(
    ...
    return bool(config.use_llm and rules is not None and cat_heur in rules.skip_llm_if_heuristic_category)
...
def _final_summary_tokens_for_context(
    ...
    if not runtime.config.use_llm or context.skip_llm_by_rule:
        return []
...
def _log_llm_metadata_result(
    ...
    if not (config.explain and config.use_llm and not context.skip_llm_by_rule):
        return
    logger.info(
...
def _llm_failed_for_context(context: _CategoryContext, config: RenamerConfig) -> bool:
    ...
    llm_failed = bool(config.use_llm and not context.skip_llm_by_rule and category_unknown)
    if llm_failed:
