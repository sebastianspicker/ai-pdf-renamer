...
def _try_vision_extraction(request: VisionExtractionRequest) -> str | None:
    ...
    prompt_fn = request.extraction_fns.prompt_fn or build_vision_filename_prompt
    sanitize_fn = request.extraction_fns.sanitize_fn or sanitize_filename_from_llm
    prompt = prompt_fn(request.config.output.naming.language)
...
def extract_pdf_content_result_with(
    ...
    *,
    ...
    if vision_first_result.content is not None:
        return ExtractionResult(vision_first_result.content, used_vision=True)
    ...
    *,
    ...
    if _should_try_vision_fallback(config, tried_vision, content_length):
        ...
        if fallback is not None:
            return ExtractionResult(fallback, used_vision=True)
...
def _ensure_llm_client(config: RenamerConfig, client: LLMClient | None) -> LLMClient:
    ...
    if client is not None:
        return client
    raise RuntimeError("Vision extraction requires a caller-supplied LLM client")
