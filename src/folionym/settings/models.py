"""Immutable grouped configuration models and flat-input assembly."""
...
def _validate_renamer_config(config: RenamerConfig) -> None:
    ...
    _validate_choice(config.output.naming.language, _VALID_LANGUAGES, "--language")
...
def build_config_from_flat_dict(data: dict[str, Any]) -> RenamerConfig:
    ...
    return RenamerConfig(
        llm=LLMConfig(
            backend=LLMBackendConfig(**group_values(LLMBackendConfig, data)),
            runtime=LLMRuntimeConfig(**group_values(LLMRuntimeConfig, data)),
            vision=LLMVisionConfig(**group_values(LLMVisionConfig, data)),
            content=LLMContentConfig(**group_values(LLMContentConfig, data)),
        ),
        heuristic=HeuristicConfig(
            scoring=HeuristicScoreConfig(**group_values(HeuristicScoreConfig, data)),
            category=HeuristicCategoryConfig(**group_values(HeuristicCategoryConfig, data)),
            skip=HeuristicSkipConfig(**group_values(HeuristicSkipConfig, data)),
            window=HeuristicWindowConfig(**group_values(HeuristicWindowConfig, data)),
        ),
        extraction=ExtractionConfig(**group_values(ExtractionConfig, data)),
        output=OutputConfig(
            naming=OutputNamingConfig(**group_values(OutputNamingConfig, data)),
            template=OutputTemplateConfig(**group_values(OutputTemplateConfig, data)),
            paths=OutputPathConfig(**group_values(OutputPathConfig, data)),
            traversal=OutputTraversalConfig(**group_values(OutputTraversalConfig, data)),
            mode=OutputModeConfig(**group_values(OutputModeConfig, data)),
            hooks=OutputHookConfig(**group_values(OutputHookConfig, data)),
            progress_options=OutputProgressConfig(**group_values(OutputProgressConfig, data)),
        ),
    )
