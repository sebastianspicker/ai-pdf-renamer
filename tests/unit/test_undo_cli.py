"""Tests for undo rename-log operations in folionym.undo_cli."""

from __future__ import annotations

import pytest

from folionym.undo_cli import main, run_undo


def _run_logged_undo(
    tmp_path, capsys, entry: tuple[str, str], *, payload: str, prefix: str = ""
) -> tuple[object, object, str]:
    """Create one logged rename, execute undo, and return paths plus captured output."""
    old = tmp_path / entry[0]
    new = tmp_path / entry[1]
    new.write_text(payload, encoding="utf-8")
    log = tmp_path / "rename.log"
    log.write_text(f"{prefix}{old}\t{new}\n", encoding="utf-8")
    main(["--rename-log", str(log)])
    return old, new, capsys.readouterr().out
...
def test_undo_single_entry(tmp_path, capsys) -> None:
    ...
    old, new, output = _run_logged_undo(tmp_path, capsys, ("original.pdf", "renamed.pdf"), payload="content")
...
def test_undo_missing_target(tmp_path, capsys) -> None:
    ...
    # Do not create 'new'; the path must remain missing.
...
def test_undo_preserves_leading_and_trailing_spaces_in_logged_paths(tmp_path, capsys) -> None:
    ...
    old, new, output = _run_logged_undo(tmp_path, capsys, (" original .pdf ", " renamed .pdf "), payload="data")
...
def test_undo_malformed_line_skipped(tmp_path, capsys) -> None:
    ...
    old, new, _ = _run_logged_undo(tmp_path, capsys, ("a.pdf", "b.pdf"), payload="data", prefix="no-tab-here\n")
