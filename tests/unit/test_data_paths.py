"""Verify packaged-data discovery and explicit override boundaries."""
...
from folionym.data_paths import (
    _discover_repo_root,
...
from folionym.llm_prompts import (
    _summary_doc_type_hint,
    build_analysis_prompt,
)
from folionym.llm_schema import (
    DEFAULT_LLM_CATEGORY,
    DEFAULT_LLM_SUMMARY,
    validate_llm_document_result,
)
from folionym.loaders import _file_mtime, load_meta_stopwords
from folionym.logging_utils import StructuredLogFormatter, setup_logging
from folionym.renamer_files import PdfCollectionOptions, _is_safe_path, collect_pdf_files
from folionym.rules import ProcessingRules, load_processing_rules
...
def test_collect_files_override(tmp_path: Path) -> None:
    ...
    result = collect_pdf_files(
        tmp_path,
        PdfCollectionOptions(files_override=[real_pdf, non_pdf]),
    )
...
def test_collect_files_override_rejects_paths_outside_root(tmp_path: Path, caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level(logging.WARNING, logger="folionym.renamer_files"):
        result = collect_pdf_files(
            root,
            PdfCollectionOptions(files_override=[inside_pdf, outside_pdf]),
        )
...
def test_collect_skip_already_named(tmp_path: Path) -> None:
    ...
    result = collect_pdf_files(tmp_path, PdfCollectionOptions(skip_if_already_named=True))
...
def test_collect_max_depth(tmp_path: Path) -> None:
    ...
    result = collect_pdf_files(tmp_path, PdfCollectionOptions(recursive=True, max_depth=1))
...
def test_collect_include_pattern(tmp_path: Path) -> None:
    ...
    result = collect_pdf_files(tmp_path, PdfCollectionOptions(include_patterns=["invoice*"]))
...
def test_collect_rejects_symlink_inside_root_for_scan_and_override(tmp_path: Path) -> None:
    """Discovery must not return a source the safe rename path will reject."""
    ...
    assert not _is_safe_path(link, tmp_path)
    assert collect_pdf_files(tmp_path) == [target]
    assert collect_pdf_files(tmp_path, PdfCollectionOptions(files_override=[link])) == []
...
def test_collect_recursive_skips_hidden(tmp_path: Path) -> None:
    ...
    result = collect_pdf_files(tmp_path, PdfCollectionOptions(recursive=True))
    names = {p.name for p in result}
    assert "visible.pdf" in names
    assert ".hidden.pdf" not in names
...
def test_collect_with_rules_filter(tmp_path: Path) -> None:
    ...
    result = collect_pdf_files(tmp_path, PdfCollectionOptions(rules=rules))
    names = {p.name for p in result}
...
def test_data_path_env_override(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """FOLIONYM_DATA_DIR env var redirects data_dir()."""
