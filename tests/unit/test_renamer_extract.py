...
from folionym.llm_backend import VisionCompletionOptions
from folionym.llm_prompts import build_vision_filename_prompt
from folionym.pdf_extract import DEFAULT_MAX_CONTENT_TOKENS
from folionym.rename_ops import sanitize_filename_from_llm
from folionym.renamer_extract import (
    ExtractionFunctions,
...
def _config(**values: object) -> RenamerConfig:
...
def test_effective_max_tokens_from_config() -> None:
    ...
    config = _config(max_tokens_for_extraction=5000)
    assert effective_max_tokens(config) == 5000


def test_effective_max_tokens_from_env(monkeypatch: pytest.MonkeyPatch) -> None:
    """Falls back to FOLIONYM_MAX_TOKENS env var when config is None."""
    monkeypatch.setenv("FOLIONYM_MAX_TOKENS", "9999")
    config = _config(max_tokens_for_extraction=None)
...
def test_effective_max_tokens_default(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.delenv("FOLIONYM_MAX_TOKENS", raising=False)
    config = _config(max_tokens_for_extraction=None)
...
def _extract_scenario(
    ...
    *,
    ...
    """Run a deterministic extraction scenario with lightweight function doubles."""
    return extract_pdf_content_with(
        Path("/fake/doc.pdf"),
        config,
        extraction_fns=_extraction_fns(
            image_fn=lambda _path: image,
            pdf_to_text_fn=lambda _path, max_pages=0, max_tokens=0: text,
            pdf_to_text_with_ocr_fn=lambda _path, max_pages=0, max_tokens=0, language="de": ocr_text,
        ),
        llm_client=client,
    )


def _vision_fallback_scenario() -> tuple[RenamerConfig, MagicMock]:
    """Return the common short-text configuration and client for fallback tests."""
    client = MagicMock()
...
def test_extract_rejects_source_symlink_before_any_strategy(tmp_path: Path) -> None:
    _target, link = make_pdf_symlink(tmp_path)
    image = MagicMock(return_value="image")
    text = MagicMock(return_value="content")
    ocr = MagicMock(return_value="content")

    with pytest.raises(OSError, match="symbolic links are not supported"):
        extract_pdf_content_with(
            link,
            _config(vision_first=True, use_vision_fallback=True),
            extraction_fns=_extraction_fns(
                image_fn=image,
                pdf_to_text_fn=text,
                pdf_to_text_with_ocr_fn=ocr,
            ),
...
def test_vision_extraction_success() -> None:
    ...
    config = _config(language="de", vision_model=None, llm_timeout_s=30.0)
    ...
    assert args == (
        "base64data",
        "describe this",
        VisionCompletionOptions(model="test-model", image_mime_type="image/jpeg", timeout_s=60.0),
    )
    assert kwargs == {}
...
def test_extract_text_only() -> None:
    ...
    content, used_vision = _extract_scenario(config, client, image=None, text="Hello world from PDF")
...
def test_extract_text_only_does_not_create_llm_client_when_vision_unused(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("folionym.renamer_extract.create_llm_client_from_config", fail_if_called)
...
def test_extract_vision_fallback() -> None:
    ...
    config, client = _vision_fallback_scenario()
    content, used_vision = _extract_scenario(config, client, image="base64img", text="short")
...
def test_extract_vision_fallback_preserves_png_mime_type() -> None:
    ...
    content, used_vision = _extract_scenario(
        config,
        client,
        image={"image_b64": "pngbase64", "mime_type": "image/png"},
        text="short",
    )
    ...
    assert args[2] == VisionCompletionOptions(model="test-model", image_mime_type="image/png", timeout_s=60.0)
    assert kwargs == {}
...
def test_extract_no_content() -> None:
    ...
    content, used_vision = _extract_scenario(config, client, image="base64img", text="")
...
def test_extract_logs_text_strategy(caplog: pytest.LogCaptureFixture) -> None:
    ...
    client = MagicMock()
    client.model = "test-model"
    config = _config(use_ocr=False, vision_first=False, use_vision_fallback=False, max_tokens_for_extraction=1000)

    with caplog.at_level(logging.INFO, logger="folionym.renamer_extract"):
        content, used_vision = _extract_scenario(config, client, image=None, text="Hello world from PDF")
