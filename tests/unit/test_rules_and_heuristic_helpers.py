...
async def test_run_log_has_bounded_retention() -> None:
    """Large batches must not retain an unbounded in-memory terminal history."""
    from textual.widgets import RichLog

    app = _make_app()
    async with app.run_test(size=(120, 40)):
        assert app.query_one("#run-log", RichLog).max_lines == 5_000
...
async def test_app_default_widget_values() -> None:
    ...
    async with app.run_test(size=(120, 40)):
        ...
        assert app.query_one("#llm_url", Input).placeholder == "http://127.0.0.1:11434/v1/completions"
        ...
        with pytest.raises(QueryError):
            app.query_one("#dry_run", Checkbox)
...
async def test_get_bool_returns_value() -> None:
    ...
    app = _make_app()
    async with app.run_test(size=(120, 40)):
        cb = app.query_one("#use_ocr", Checkbox)
        cb.value = False
        assert app.get_bool("use_ocr") is False
        cb.value = True
        assert app.get_bool("use_ocr") is True
...
async def test_load_settings_populates_widgets() -> None:
    ...
    try:
        with patch("folionym.tui_state.SETTINGS_PATH", tf_path):
            app = FolionymTUI()
            ...
            async with app.run_test(size=(120, 40)):
                ...
                with pytest.raises(QueryError):
                    app.query_one("#dry_run", Checkbox)
                assert app.snapshot()["dry_run"] is True
