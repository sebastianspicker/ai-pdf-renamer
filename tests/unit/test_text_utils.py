"""Verify text normalization, date extraction, token handling, and case conversion."""
...
from folionym.text_utils import (
...
def test_extract_date_from_content_ymd() -> None:
    assert extract_date_from_content("Hello 2024-01-09 world", today=REFERENCE_TODAY) == "2024-01-09"


def test_extract_date_from_content_dmy() -> None:
    assert extract_date_from_content("Hello 9.1.2024 world", today=REFERENCE_TODAY) == "2024-01-09"
...
def test_extract_date_ymd_format() -> None:
    ...
    assert extract_date_from_content("Date: 2025-03-15", today=REFERENCE_TODAY) == "2025-03-15"


def test_extract_date_dmy_dotted() -> None:
    """DD.MM.YYYY format with two-digit day and month."""
    assert extract_date_from_content("am 15.03.2025 erstellt", today=REFERENCE_TODAY) == "2025-03-15"


def test_extract_date_german_long_form() -> None:
    """German long-form: '18. Februar 2025'."""
    assert extract_date_from_content("Vom 18. Februar 2025", today=REFERENCE_TODAY) == "2025-02-18"


def test_extract_date_german_long_form_maerz() -> None:
    """German long-form with März (special umlaut month)."""
    assert extract_date_from_content("am 5. März 2024 gesendet", today=REFERENCE_TODAY) == "2024-03-05"


def test_extract_date_english_long_form() -> None:
    """English long-form: 'February 18, 2025'."""
    assert extract_date_from_content("Dated February 18, 2025", today=REFERENCE_TODAY) == "2025-02-18"


def test_extract_date_english_long_form_no_comma() -> None:
    """English long-form without comma: 'March 5 2024'."""
    assert extract_date_from_content("On March 5 2024 we agreed", today=REFERENCE_TODAY) == "2024-03-05"


def test_extract_date_english_dd_month_year() -> None:
    """English day-first long-form: '18 February 2025'."""
    assert extract_date_from_content("Published 18 February 2025", today=REFERENCE_TODAY) == "2025-02-18"


def test_extract_date_month_year_only_german() -> None:
    """German month-year only: 'Januar 2025' -> 2025-01-01."""
    assert extract_date_from_content("Ausgabe Januar 2025", today=REFERENCE_TODAY) == "2025-01-01"


def test_extract_date_month_year_only_english() -> None:
    """English month-year only: 'March 2024' -> 2024-03-01."""
    assert extract_date_from_content("Report for March 2024", today=REFERENCE_TODAY) == "2024-03-01"


def test_extract_date_prefix_datum() -> None:
    """Prefix pattern: 'Datum: DD.MM.YYYY'."""
    assert extract_date_from_content("Datum: 15.01.2025", today=REFERENCE_TODAY) == "2025-01-15"


def test_extract_date_prefix_stand() -> None:
    """Prefix pattern: 'Stand: DD.MM.YYYY'."""
    assert extract_date_from_content("Stand: 01.06.2024", today=REFERENCE_TODAY) == "2024-06-01"
...
def test_extract_date_prefix_ymd_before_prefix() -> None:
    ...
    assert extract_date_from_content(text, today=REFERENCE_TODAY) == "2025-01-15"
...
def test_extract_date_multiple_dates_first_wins() -> None:
    ...
    assert extract_date_from_content(text, today=REFERENCE_TODAY) == "2024-06-01"
...
def test_extract_date_invalid_month_day_returns_today() -> None:
    ...
    # Month 13 and day 32 are invalid, so make_ymd will catch ValueError.
    assert extract_date_from_content("Date 2025-13-32", today=REFERENCE_TODAY) == "2026-04-08"
...
def test_extract_date_mdy_locale() -> None:
    ...
    result = extract_date_from_content("Date 03.15.2025", today=REFERENCE_TODAY, date_locale="mdy")
...
def test_extract_date_mdy_locale_ambiguous() -> None:
    ...
    result = extract_date_from_content("01.12.2025", today=REFERENCE_TODAY, date_locale="mdy")
...
def test_extract_date_prefer_leading_chars_not_in_header() -> None:
    ...
    assert extract_date_from_content(text, today=REFERENCE_TODAY, prefer_leading_chars=100) == "2025-08-20"
...
def test_extract_date_iso8601_timestamp() -> None:
    ...
    assert extract_date_from_content(text, today=REFERENCE_TODAY) == "2025-03-15"
...
def test_extract_date_keyword_weight_prefers_document_date() -> None:
    ...
    assert extract_date_from_content(text, today=REFERENCE_TODAY, prefer_leading_chars=500) == "2025-03-15"
...
def test_extract_date_short_month_name_english() -> None:
    ...
    assert extract_date_from_content("Statement dated Sep 5, 2024", today=REFERENCE_TODAY) == "2024-09-05"
...
def test_structured_fields_amount_preserves_single_decimal_digit() -> None:
    """One-decimal amounts keep their decimal separator instead of collapsing digits."""
    text = "Total: 12.3 EUR\nSubtotal: 12,3 EUR"
    out = extract_structured_fields(text)
    assert out["amount"] == "12.3"
