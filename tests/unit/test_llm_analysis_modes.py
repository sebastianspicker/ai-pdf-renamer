"""Exercise single-call analysis, chat transport, JSON mode, and LLM presets."""
...
from folionym.llm_backend import HttpLLMBackend, create_llm_client_from_config
from folionym.llm_options import AnalysisOptions, JsonCompletionOptions, LlmPromptOptions
from folionym.llm_schema import DocumentAnalysisResult

REFERENCE_TODAY = date(2026, 4, 8)

# ---------------------------------------------------------------------------
# Single-call document analysis
# ---------------------------------------------------------------------------
...
def test_get_document_analysis_parses_valid_json():
    from folionym.llm import get_document_analysis
    ...
    result = get_document_analysis(
        ...
        AnalysisOptions(prompt=LlmPromptOptions(language="en")),
    )
...
def test_get_document_analysis_returns_defaults_on_empty_content():
    ...
    result = get_document_analysis(client, "", AnalysisOptions(prompt=LlmPromptOptions(language="en")))
...
def test_get_document_analysis_returns_defaults_on_short_content():
    ...
    client = FakeLLMClient("")
    result = get_document_analysis(client, "short", AnalysisOptions(prompt=LlmPromptOptions(language="en")))
    assert result.summary == "na"
...
@pytest.mark.parametrize(
    ("json_mode", "expected_response_format"),
    [(True, {"type": "json_object"}), (False, None)],
    ids=["enabled", "default-disabled"],
)
def test_get_document_analysis_json_mode(json_mode: bool, expected_response_format: dict[str, str] | None) -> None:
    from folionym.llm import get_document_analysis
    ...
    get_document_analysis(
        client,
        "This is a long document content " * 10,
        AnalysisOptions(prompt=LlmPromptOptions(language="en"), json_mode=json_mode),
    )
    assert client.calls[0]["response_format"] == expected_response_format
...
def test_factory_creates_chat_backend_by_default():
    config = build_config_from_flat_dict({"use_llm": True})
...
def test_factory_creates_text_backend_when_chat_disabled():
    config = build_config_from_flat_dict({"use_llm": True, "llm_use_chat_api": False})
    client = create_llm_client_from_config(config)
    assert isinstance(client, HttpLLMBackend)
...
def test_complete_json_with_retry_json_mode_reduces_retries():
    from folionym.llm import complete_json_with_retry
...
def test_complete_json_with_retry_normal_mode_uses_max_retries():
    ...
    client = FakeLLMClient("not json")
    complete_json_with_retry(client, "prompt", JsonCompletionOptions(json_mode=False, max_retries=3))
...
def test_complete_json_with_retry_json_mode_passes_response_format():
    ...
    client = FakeLLMClient(response)
    complete_json_with_retry(client, "prompt", JsonCompletionOptions(json_mode=True))
    ...
    name, _meta = filename_mod.generate_filename(
        ...
        FilenameGenerationRequest(
            config=build_config_from_flat_dict(
                {"language": "en", "desired_case": "kebabCase", "use_single_llm_call": True}
            ),
            dependencies=FilenameGenerationDependencies(
                llm_client=FakeLLMClient(""), heuristic_scorer=scorer, stopwords=Stopwords(words=set())
            ),
...
def test_config_has_new_fields():
    ...
    assert config.llm.runtime.use_single_llm_call is True
    assert config.llm.runtime.llm_use_chat_api is True
    assert config.llm.runtime.llm_json_mode is True


def test_config_new_fields_can_be_disabled():
    config = build_config_from_flat_dict(
        {"use_single_llm_call": False, "llm_use_chat_api": False, "llm_json_mode": False}
    )
    assert config.llm.runtime.use_single_llm_call is False
    assert config.llm.runtime.llm_use_chat_api is False
    assert config.llm.runtime.llm_json_mode is False
...
def test_preset_apple_silicon_defaults():
    ...
    assert cfg.llm.backend.llm_model == "qwen2.5:3b"
    assert cfg.llm.backend.llm_base_url == "http://127.0.0.1:11434/v1/completions"
    assert cfg.llm.content.max_context_chars == 120_000
    assert cfg.llm.backend.llm_preset == "apple-silicon"
...
def test_preset_gpu_defaults():
    ...
    assert cfg.llm.backend.llm_model == "qwen2.5:7b-instruct"
    assert cfg.llm.backend.llm_base_url == "http://127.0.0.1:11434/v1/completions"
    assert cfg.llm.content.max_context_chars == 480_000
    assert cfg.llm.backend.llm_preset == "gpu"
...
def test_preset_gpu_explicit_model_overrides():
    ...
    assert cfg.llm.backend.llm_model == "custom-model"
    assert cfg.llm.content.max_context_chars == 480_000
...
def test_invalid_preset_warns_and_falls_back(caplog: pytest.LogCaptureFixture) -> None:
    ...
    with caplog.at_level("WARNING", logger="folionym.config_resolver"):
...
def test_explicit_max_content_chars_overrides_preset():
    ...
    assert cfg.llm.content.max_context_chars == 200_000
...
def test_explicit_llm_url_overrides_preset():
    ...
    assert cfg.llm.backend.llm_base_url == "http://custom:9999/v1/completions"
